...
// MARK: - Pending suggestion slot

/// 某一维度尚无 AI 条目时占位（不使用默认 / mock 文案）。
private struct SuggestionPendingCardView: View {
    let dimension: SuggestionDimension
    var isLoading: Bool

    private var cardSeed: UInt64 {
        dimension.rawValue.utf8.reduce(UInt64(0)) { $0 &+ UInt64($1) }
    }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 6) {
            HStack(spacing: 6) {
                Image(systemName: dimension.iconName)
                    ...
                    .frame(width: 26, height: 26)
                    .background(Circle().fill(dimension.color.opacity(0.12)))
            ...
            VStack(alignment: .leading, spacing: 4) {
                if isLoading {
                    NotebookLinesLoadingView(style: .compact)
                } else {
                    Text("home.suggestion.empty_dimension")
                        .font(Theme.itim(size: 18))
                        .foregroundStyle(Color.black.opacity(0.50))
                        .lineLimit(4)
                        .fixedSize(horizontal: false, vertical: true)
        ...
        .grainCardStyle(seed: cardSeed)
        .accessibilityLabel(
            isLoading
            ? "\(dimension.displayName)。\(String(localized: "home.suggestion.refreshing_a11y"))"
            : "\(dimension.displayName)。\(String(localized: "home.suggestion.empty_dimension_a11y"))"
        )
    }
}
...
struct SuggestionCardView: View {
    ...
    var body: some View {
        ZStack(alignment: .topTrailing) {
            VStack(alignment: .leading, spacing: 6) {
                HStack(spacing: 6) {
                    Image(systemName: suggestion.dimension.iconName)
                        .font(.system(size: 13))
                        .foregroundStyle(suggestion.dimension.color)
                        .frame(width: 26, height: 26)
                        .background(Circle().fill(suggestion.dimension.color.opacity(0.12)))
                    Text(suggestion.dimension.displayName)
                        .font(Theme.itim(size: 22))
                        .foregroundStyle(Theme.textPrimary)
                }
                ...
                VStack(alignment: .leading, spacing: 4) {
                    ...
                    if let first = suggestion.details.first {
                        Text(first)
                            .font(Theme.itim(size: 18))
                            .lineSpacing(Theme.lineSpacing)
                            .foregroundStyle(Color.black.opacity(0.50))
                ...
                if !suggestion.citationSources.isEmpty {
                    HStack(spacing: 2) {
                        Image(systemName: "book.closed.fill")
                            .font(.system(size: 7))
                        Text("home.reference_badge")
            ...
            if isLoading {
                MiniEllipsisLoadingView()
                    .padding(.top, 2)
                    .padding(.trailing, 2)
            }
        }
        .grainCardStyle(seed: suggestion.dimension.rawValue.utf8.reduce(UInt64(0)) { $0 &+ UInt64($1) })
        .accessibilityLabel(
            isLoading
            ? "\(suggestion.title)。\(String(localized: "home.suggestion.refreshing_a11y"))"
            : suggestion.title
        )
    }
}
...
struct SuggestionDetailView: View {
    ...
    var body: some View {
        ZStack {
            Theme.background
                .grainTexture(intensity: .subtle, seed: 300)
                .ignoresSafeArea()
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 20) {
                    ...
                    VStack(alignment: .leading, spacing: 12) {
                        ForEach(Array(suggestion.details.enumerated()), id: \.offset) { index, detail in
                            HStack(alignment: .top, spacing: 10) {
                                Text("\(index + 1)")
                                    .font(Theme.itim(size: 14))
                                    .foregroundStyle(.white)
                                    .frame(width: 22, height: 22)
                                    .background(Circle().fill(suggestion.dimension.color))
                    ...
                    .background(
                        Theme.cardBackgroundSolid
                            .overlay(GrainOverlay(intensity: .light, seed: 77))
        ...
        .navigationTitle(suggestion.dimension.displayName)
        .navigationBarTitleDisplayMode(.inline)
